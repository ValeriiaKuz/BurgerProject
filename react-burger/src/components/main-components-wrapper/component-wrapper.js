...
const ComponentWrapper = () => {
    ...
    const [orderPriceState, orderPriceDispatcher] = useReducer(reducer, orderPriceInitialState, undefined);
    ...
    return (
        <main>
            ...
            <section className={style.ingredients}>
                <BurgerIngredients getAddedIngredients={getAddedIngredients}
                ...
                <div className={style.orderPrice + ' ' + 'pr-4 pt-10'}>
                    ...
                    {isOpenModal &&
                        <Modal onClose={handleCloseModal}>
                            <div>
                                {isError && <div className='m-5'> Ошибка: что-то пошло не так.</div>}
                                {isLoading && <div className='m-5'> Загрузка</div>}
                                {!isError && !isLoading && orderID && <OrderDetails orderID={orderID}/>}
                            </div>
                        </Modal>}
                </div>
            </section>
